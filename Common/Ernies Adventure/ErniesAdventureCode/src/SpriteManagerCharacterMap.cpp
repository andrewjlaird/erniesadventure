...
Character* SpriteManager::parseCharacterFile(std::ifstream& scenefile, std::string sceneName)
{
   Character* newCharacter = NULL;
   ...
   if (scenefile.is_open())
   {
      ...
      {
         // (U) Don't use the brain so clean up the memory
...
LayeredObject* SpriteManager::getSpriteLayeredObjectById(SpriteIdType id)
{
   LayeredObject* toReturn = NULL;
...
LayeredObject* SpriteManager::getCharacterLayeredObjectById(CharacterIdType id)
{
   LayeredObject* toReturn = NULL;
   ...
   for (int i = 0 ; i < characters.size() ; i++)
   {
      ...
      {
         toReturn = characters[i]->getLayeredObject();
         break;
      }
   }
   
   for (int i = 0 ; toReturn == NULL && i < basicCharacters.size() ; i++)
   {
      if (basicCharacters[i]->getId() == id)
      {
         toReturn = basicCharacters[i]->getLayeredObject();
         break;
      }
   }
...
unsigned long SpriteManager::registerCharacter(Character* newCharacter, CharacterIdType desiredCharacterId)
{
   ...
   {
      basicCharacters.insert(std::pair<CharacterIdType, Character*>(charId, newCharacter));
   }
   else
   {
      characters.insert(std::pair<CharacterIdType, Character*>(charId, newCharacter));
   }
...
void SpriteManager::releaseCharacterWithId(CharacterIdType characterId)
{
   ...
   std::map<CharacterIdType, Character*>::iterator iter = characters.find(characterId);
   ...
   iter = basicCharacters.find(characterId);

   if (iter != basicCharacters.end())
   {
      found = true;
      basicCharacters.erase(characterId);
...
void SpriteManager::setSpriteInGame(SpriteIdType id, int x, int y)
{
   bool found = false;
   for (int i = 0 ; i < sprites.size() ; i++)
   {
...
bool SpriteManager::isCharacterInCurrentScene(CharacterIdType characterToCheck)
{
   ...
   std::map<CharacterIdType, Character*>::iterator iter = characters.find(characterToCheck);
   ...
   iter = basicCharacters.find(characterToCheck);
...
Point SpriteManager::getCharacterPoint(CharacterIdType id)
{
   ...
   std::map<CharacterIdType, Character*>::iterator iter = characters.find(id);
...
Point SpriteManager::getSpritePoint(SpriteIdType id)
{
   Point point(-1, -1);
   bool found = false;   
   for (int i = 0 ; i < sprites.size() ; i++)
   {
      ...
      {
         found = true;
         point = Point(static_cast<int>(sprites[i]->getX()), static_cast<int>(sprites[i]->getY()));
         break;
      }
   }
   ...
   for (int i = 0 ; !found && i < basicSprites.size() ; i++)
   {
      if (basicSprites[i]->getId() == id)
      {
         point = Point(static_cast<int>(basicSprites[i]->getX()), static_cast<int>(basicSprites[i]->getY()));
         break;
      }
   }
...
void SpriteManager::timeChange(double dt)
{
   ...
   for (std::map<CharacterIdType, Character*>::iterator i = characters.begin() ; i != characters.end() ; i++)
   {
      i->second->timeChange(dt);
   }
}
...
bool SpriteManager::canMoveHere(int x, int y, CharacterIdType characterId)
{
   ...
   for (std::map<CharacterIdType, Character*>::iterator i = characters.begin() ; canMoveHere && i != characters.end() ; i++)
   {
      if (i->second != NULL &&
          i->first != characterId &&
          i->second->isOnScreen() &&
          i->second->getLayeredObject() != NULL)
      {
         canMoveHere = i->second->getLayeredObject()->canMoveHere(x, y);
      }
   }
...
bool SpriteManager::canMoveHere(Square toCheck, CharacterIdType characterId)
{
   ...
   for (std::map<CharacterIdType, Character*>::iterator i = characters.begin() ; canMoveHere && i != characters.end() ; i++)
   {
          ...
          i->second->getLayeredObject() != NULL)
      {
         ...
         if (i->second->getLayeredObject()->getBoundarySquare().upperLeft.x != -1)
         {
            // (U) We can move here, if the two characters don't overlap
            canMoveHere = !i->second->getLayeredObject()->getOffsetBoundarySquare().overlap(toCheck);
         }
      }
   }
...
int SpriteManager::getMainCharacterX()
{
   ...
   for (std::map<CharacterIdType, Character*>::iterator i = characters.begin() ; i != characters.end() ; i++)
   {
      if (i->second->isMainCharacter())
      {
         found = true;
         x = static_cast<int>(i->second->getX());
         break;
      }
   }

   for (std::map<CharacterIdType, Character*>::iterator i = basicCharacters.begin() ; !found && i != basicCharacters.end() ; i++)
   {
      if (i->second->isMainCharacter())
      {
         found = true;
...
int SpriteManager::getMainCharacterY()
{
   ...
   for (std::map<CharacterIdType, Character*>::iterator i = characters.begin() ; i != characters.end() ; i++)
   {
      ...
      {
         found = true;
         y = static_cast<int>(i->second->getY());
         break;
      }
   }
